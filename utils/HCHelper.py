...
class HCHelper(object):
	...
	def kunci(n, x0):
		...
		banyak = int(n * (n - 1)/2)
		barisan = HCHelper.log(x0, banyak + n - 1)
		...
		for baris_i in range(1,n):
			msa[baris_i] = HCHelper.r_ij(msa, baris_i, 0, barisan[indeks])%55000
			indeks += 1
			# ~ print(msa)
			# ~ print('-'*10)
		# ~ print('='*20)
		...
		for kolom in range(n):
			for baris in range(kolom+1,n):
				augmented[baris] = HCHelper.r_ij(augmented, baris, kolom, -augmented[baris, kolom])%55000
	...
	def enkripsi(nama_plainteks):
		...
		kunciku, balikku = HCHelper.kunci(ukuran, x0)
		...
		print('Your encrypted message saved in file: results/enkripsi.txt')
		print('Password saved in file: results/password_enkripsi.txt')
	...
	def dekripsi(nama_chiperteks):
		...
		print(f'Decryption time: {selama} seconds')
...
# 	# plainteks = input('Please enter the ciphertext file name and its extension: ')

# 	#proses enkripsi dimulai
# 	#enkripsi hill cipher
# 	enkripsi("loremm.txt") #hasil disimpan di file enkripsi.txt

# 	with open('enkripsi.txt', 'r') as file:
# 		msg1 = file.read()
# 		msg1 = msg1.encode()
...
# 	# chiperteks = input('Please enter the ciphertext file name and its extension: ')
	
# 	#proses dekripsi dimulai
# 	private_key = "private.pem"

# 	#dekripsi RSA
# 	with open('rsa_enkripsi.txt', 'r') as file:
# 		encrypted = file.read()
# 		encrypted = encrypted.encode()

# 	decrypted = RSAHelper.decrypt(b64decode(encrypted), private_key)
# 	f = open ('rsa_dekripsi.txt', 'w',encoding='utf-8')
# 	f.write(str(decrypted.decode()))
# 	f.close()
	
# 	#dekripsi hillcipher
# 	dekripsi('rsa_dekripsi.txt')
